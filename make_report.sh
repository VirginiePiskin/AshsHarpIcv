...
export PATH=/home/xnat/pauly/R/install/bin:$PATH
export R_LIBS=/home/xnat/pauly/R/Rlibs
export RSTUDIO_PANDOC=/var/chead-apps/apps/pandoc/1.12.4.2-in-rstudio
...
# Read the options
while getopts "s:n:t:a:c:h:w:" opt; do
...
rmdf=$REPORTDIR/clinical_report.Rmd
ndf=$REPORTDIR/ADNI_metadata_for_R.csv
...
icv_layer=$WORKDIR/${full_id}_icv.nii.gz
harp_layer=$WORKDIR/${full_id}_harp.nii.gz
...
if [[ $extent ]] ; then
	...
	echo "$session_id ICV $extent $icv" > $WORKDIR/${full_id}_icv_volumes.txt
fi 
...
if [[ $extent ]] ; then
	hvl=$(c3d $harp_layer -dup -lstat | awk -v id=102 '$1 == id {print $7}')
        echo "$session_id HVL $extent $hvl" > $WORKDIR/${full_id}_harp_left_volumes.txt
fi
...
if [[ $extent ]] ; then
	hvr=$(c3d $harp_layer -dup -lstat | awk -v id=103 '$1 == id {print $7}')
        echo "$session_id HVR $extent $hvr" > $WORKDIR/${full_id}_harp_right_volumes.txt
fi
...
qaicv=$WORKDIR/multiatlas_corr_nogray_icv_qa.png
qahvl=$WORKDIR/bootstrap_corr_nogray_harp_left_qa.png
qahvr=$WORKDIR/bootstrap_corr_nogray_harp_right_qa.png
...
qaicv_croped=$WORKDIR/${full_id}_qa_icv.png
qahvl_croped=$WORKDIR/${full_id}_qa_harp_left.png
qahvr_croped=$WORKDIR/${full_id}_qa_harp_right.png
...
R -e "file.rename('$REPORTDIR/$report','$WORKDIR/$report')"
R -e "file.rename('$REPORTDIR/$tex','$WORKDIR/$tex')"
